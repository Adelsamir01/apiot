#!/usr/bin/env python3
"""run_benchmark.py — Stepwise experiment harness for APIOT.

Requires IoT Virtual Lab (iot_vlab) to be running with targets already up.
APIOT does NOT start, stop, or respawn the lab.

Usage (operator or automation drives each step):
  1. python3 scripts/run_benchmark.py step
     -> Mapper runs, prints targets, then exits.
  2. python3 scripts/run_benchmark.py step --command "attack ..."
     -> Executes one red-phase action and exits.
  3. python3 scripts/run_benchmark.py step --command "verify_crash <ip>"
     -> Verifies impact and exits.
  4. python3 scripts/run_benchmark.py step --command "done"
     -> Completes the scenario (blue phase / metrics) and advances.
"""
...
def execute_command(cmd_str: str) -> str:
    """Parse and execute command. Returns output for operator."""
