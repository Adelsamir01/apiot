...
Requires iot_vlab to be running with a zephyr_coap sensor already spawned.
APIOT does NOT start, stop, or respawn iot_vlab.

Sequence:
  1. CHECK:     Verify lab is online and CoAP sensor is reachable.
...
def run_test(client: LabClient, memory: AgentMemory, logger: AttackLogger):
    ...
    # ── STEP 1: CHECK LAB ────────────────────────────────────────────
    print("\n" + "="*60)
    print("  STEP 1: VERIFY LAB HAS CoAP SENSOR")
    print("="*60)

    topo = client.get_topology()
    coap_dev = next((d for d in topo if d["firmware_id"] == "zephyr_coap"), None)
    check("zephyr_coap in topology", coap_dev is not None,
          "Spawn zephyr_coap via iot_vlab before running this test")
    if not coap_dev:
        return

    check("zephyr_coap alive", coap_dev.get("alive"))
    ...
    if not has_ip:
        print("[!] Sensor has no IP. Wait for DHCP and retry.")
        return
